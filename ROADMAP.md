...
- [In Progress] Web engine prototype — now living in the [Omoluabi-News repository](https://github.com/ukadike/omoluabi-news) at `engine/` (moved from this repo's `web-engine/app/` per Kemi's 2026-07-08 direction): a browser-only human review interface over the governance pipeline, gated in the order of `architecture/governance-pipeline.md`, plus a manual "New observation" form with a required headline (`#/new`, `source.origin_type: "web-form"`) and a news preview (`#/news`), so the full Observation → Consent → Source → Risk → Accessibility → Human Review → Publication Status → Archive → Read loop runs end to end without a field device. Local-only (IndexedDB), no backend, no production framework decision, AI assist is a labeled stub. Planning docs stay here in `web-engine/`; see `web-engine/local-first-plan.md` for what remains open (sync, production storage, encryption-at-rest).
