...
- [Done — retired] Web engine browser prototype — a browser-only human review interface over the governance pipeline, gated in the order of `architecture/governance-pipeline.md`, plus a manual "New observation" form with a required headline (`#/new`, `source.origin_type: "web-form"`) and a news preview (`#/news`), proving the full Observation → Consent → Source → Risk → Accessibility → Human Review → Publication Status → Archive → Read loop runs end to end without a field device. Built here as `web-engine/app/`, moved to the [Omoluabi-News repository](https://github.com/ukadike/omoluabi-news) per Kemi's 2026-07-08 direction, retired 2026-08-16 and preserved intact (still runnable) at its `archive/engine-browser-prototype/`.
- [In Progress] Web engine, successor plan — designated by Kemi, superseding the browser prototype. Kemi's planning conversation (2026-08-12, shared 2026-08-18) is preserved in the Omoluabi-News repo at `engine/sources/`, with draft spec documents reconstructed from it in its `engine/docs/` and `engine/tasks/` (per-section provenance markers; the two original uploaded files remain `AWAITING FRAGMENT`). Open questions — including a muted-token value conflict — are recorded in that repo's `engine/README.md` for Kemi's ruling. Planning docs from the prototype era stay here in `web-engine/` as provenance.
