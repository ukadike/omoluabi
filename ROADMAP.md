...
- [In Progress] Web engine, successor plan — designated by Kemi (2026-08-16), superseding the browser prototype. Specification documents are not yet physically present in any repository; indexed as `AWAITING FRAGMENT` in the Omoluabi-News repo's `engine/README.md`, with open questions (including a muted-token value conflict) recorded there for Kemi's ruling. Planning docs from the prototype era stay here in `web-engine/` as provenance.
