...
Revises `device/parts-list.md` and `device/circuit-notes.md` by formalizing what those documents already implied (a camera-capable module, an "optional GPS module") and adding the new requirements the Photographic and Cartographic Witness Layers need. This document does not replace `device/parts-list.md` or `device/circuit-notes.md`; it is the instrumentation-specific companion referenced from both. `diagrams/device-architecture.mmd` is updated alongside this document to show the new GNSS receiver, magnetometer, and pressure sensor as inputs to the Arduino UNO Q, and the observation package now branching into Photographic Witness and Cartographic Witness records. `diagrams/circuit-directions.mmd` is a new block-level wiring diagram — which component talks to the Arduino UNO Q over which bus/signal type (I2C, UART, PWM, digital I/O) — rendering `device/circuit-notes.md`'s existing text-only circuit-direction list visually, with the new sensors added.

**No wired, pin-numbered circuit schematic accompanies this revision.** `device/circuit-notes.md` already discloses that no MVP component — old or new — has one yet; adding pin-level detail only for the new sensors would overstate their readiness relative to the rest of the device plan, and no specific GNSS receiver, magnetometer, or pressure sensor model has been selected yet to have a pinout in the first place. `diagrams/circuit-directions.mmd` stays at the same bus/signal-type level of detail as everything else in `circuit-notes.md`.
