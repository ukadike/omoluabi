...
Design documentation only. Nothing described here is implemented. The web engine's browser prototype (retired 2026-08-16, preserved in the [Omoluabi-News repository](https://github.com/ukadike/omoluabi-news) at `archive/engine-browser-prototype/`) never included map views, and the successor engine's specification is `AWAITING FRAGMENT` (see that repo's `engine/README.md`). See `web-engine/screens.md` for the prototype-era review screens.
